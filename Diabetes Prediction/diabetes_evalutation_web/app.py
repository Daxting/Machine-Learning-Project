import joblib
model_simple_pretrained = joblib.load('diabetes_model_ForCorrFeature(012)new.pkl')
#model_simple_pretrained = joblib.load('diabetes_model_simple.pkl')
#['HighBP', 'HighChol', 'BMI', 'GenHlth', 'DiffWalk']
model_complex_pretrained = joblib.load('diabetes_model_ForAllFeature(012)LBGM.pkl')
...
def submit():
    if request.method == 'POST':
        ...
        if predicted_class == 2:
            prediction = f'糖尿病高風險 - 系統信心 {100*class_probabilities[2]:.10f} %'
        elif predicted_class == 1:
            prediction = f'糖尿病低風險 - 系統信心 {100*class_probabilities[1]:.10f} %'
        else:
            prediction = f'無糖尿病風險 - 系統信心 {100*class_probabilities[0]:.10f} %'
